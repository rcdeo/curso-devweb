...
<section  ng-controller="cart-controller">
	...
	<div class="container">
		...
		<div class="row text-center title-default-roxo title-uppercase" style="margin:40px auto;">
		...
		<table id="cart-products" class="table table-bordered">
			...
			<tbody>
				<tr ng-repeat="produto in produtos">
					<td class="text-center"><img src="images/produtos/{{produto.foto_principal}}"></td>
					<td>{{produto.nome_prod_long}}</td>
					<td class="col-xs-2">
						<div class="input-group">
					      <span class="input-group-btn">
					        <button class="btn text-roxo-shop" ng-click="removeQtd(produto)" type="button"><i class="fa fa-chevron-down"></i></button>
					      </span>
					      <input type="text" class="form-control" ng-model="produto.qtd_car">
					      <span class="input-group-btn">
					        <button class="btn text-roxo-shop" ng-click="addQtd(produto)" type="button"><i class="fa fa-chevron-up"></i></button>
					      </span>
					    </div>
					</td>
					<td class="text-center col-xs-2">
						<p>Entrega para o<br/>CEP: {{carrinho.cep}}</p>
						<strong class="text-roxo-shop">{{carrinho.prazo}}</strong>
					</td>
					<td class="text-center">R$ {{produto.preco}}</td>
					<td class="text-center">R$ {{produto.total}}</td>
					<td class="text-center"><button ng-click="removeAll(produto)" class="btn text-roxo-shop" type="button"><i class="fa fa-close"></i></button></td>
				</tr>
			</tbody>
		</table><!-- cart-products -->
				...
				<div class="box-cart-totais">
					<table class="table">
						...
						<tr style="border-top:#999 1px solid;">
							<td class="text-roxo-shop text-bold">Total:</td>
							<td class="text-roxo-shop text-bold text-right">R$ {{carrinho.total}}</td>
						</tr>
					</table>
				</div>
			</div>
